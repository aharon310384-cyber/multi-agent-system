...
function buildOrchestratorPrompt(opts = {}) {
  return [
    ...
    'У тебя в подчинении 8 департаментов: Маркетинг, Дизайн, Продажи, ИТ, Управление, Копирайтинг, Безопасность, Онлайн разведка.',
    '',
    'КАК ТЫ РАБОТАЕШЬ',
    ...
    '— Дизайн, UI, макеты, презентации → designer',
    '— SEO → seo · реклама/таргет → targetolog · SMM → smm · стратегия → strategist · аналитика → analyst',
    '— Frontend → frontend · Backend → backend · Mobile → mobile · AI/ML → aiml · Data → data · DevOps → devops · QA → qa · архитектура → techlead · уязвимости → security',
    '— Продажи → sales · сопровождение → account',
